...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesSumInDays = new HashMap<>();
        for (UserMeal meal : meals) {
            caloriesSumInDays.merge(meal.getDateTime().toLocalDate(), meal.getCalories(), Integer::sum);
        }
            ...
            if (TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                userMealWithExcessList.add(
                        new UserMealWithExcess(meal.getDateTime(), meal.getDescription(),
                                meal.getCalories(), caloriesSumInDays.get(meal.getDateTime().toLocalDate()) > caloriesPerDay)
                );
            }
        }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesSumInDays = meals.stream()
        ...
        return meals.stream()
                ...
                .map(userMeal ->
                        new UserMealWithExcess(userMeal.getDateTime(), userMeal.getDescription(),
                                userMeal.getCalories(), caloriesSumInDays.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay))
